...
import sys
import glob
from augmentation_functions import get_bbox_from_file, crop, flip, blur,  \
...
def main():
    ...
    # imgs_to_skip = load_filenames('../test.txt')    
    # prefix = 'D:\\UNIVERSITA\\Magistrale\\SecondoAnno\\VisualInformationProcessingAndManagement\\ProgettoVISUAL/data/new_dataset\\'

    for img_name in image_names: 
        
        # if ('img' + img_name.lstrip(prefix)) not in imgs_to_skip: # Salva quelle da aumentare
        # if ('img' + img_name.lstrip(prefix)) in imgs_to_skip: # Salva quelle da NON aumentare
        if True:
            ...
            if(AUGMENT): 
                ...
                # # scale = 0.5 - direction = "north-west"
                # img_crop, bbox_crop = crop(img, bbox_values, 0.5, "nw")
                # # Salva in memoria i risultati del CROPPING
                # s_path = destination_folder + image_name + 'crop_nw_05'
                # if SAVE:
                #     save(img_crop, bbox_crop, s_path, r, c)
                    
                # # scale = 0.5 - direction = "north-est"
                # img_crop, bbox_crop = crop(img, bbox_values, 0.5, "ne")
                # # Salva in memoria i risultati del CROPPING
                # s_path = destination_folder + image_name + 'crop_ne_05'
                # if SAVE:
                #     save(img_crop, bbox_crop, s_path, r, c)
                    
                # # scale = 0.5 - direction = "south-west"
                # img_crop, bbox_crop = crop(img, bbox_values, 0.5, "sw")
                # # Salva in memoria i risultati del CROPPING
                # s_path = destination_folder + image_name + 'crop_sw_05'
                # if SAVE:
                #     save(img_crop, bbox_crop, s_path, r, c)
                    
                # # scale = 0.5 - direction = "south-est"
                # img_crop, bbox_crop = crop(img, bbox_values, 0.5, "se")
                # # Salva in memoria i risultati del CROPPING
                # s_path = destination_folder + image_name + 'crop_se_05'
                # if SAVE:
                #     save(img_crop, bbox_crop, s_path, r, c)
                
                # # Mostra il risultato del cropping
                # img_crop_bb = draw_bbox(img_crop, bbox_crop)
                # n_bboxes = len(bbox_values) // 4
                # n_bboxes_crop = len(bbox_crop) // 4
                # print(f"\n Bounding boxes presenti nell'immagine '{image_name}.jpg' : {n_bboxes}")
                # print(f" Bounding boxes presenti nell'immagine dopo il cropping : {n_bboxes_crop}")
                # result = np.zeros((r * 2, c * 2, ch), dtype = np.uint8)
                # result[:r, :c, :] = img
                # result[:r, c:, :] = img_bb
                # result[r:, :c, :] = img_crop
                # result[r:, c:, :] = img_crop_bb
                # show_img("Risultato cropping", result)
                
                # # Mostra il risultato del cropping per tutte e 4 le direzioni possibili
                # result = np.zeros((r * 3 + 6, c * 4 + 9, ch), dtype = np.uint8)
                # result[:r, c+3:c*2 +3, :]            = img
                # result[:r, c*2 +6:c*3 +6, :]         = img_bb
                # result[r+3:r*2 +3, :c, :]            = img_crop1
                # result[r+3:r*2 +3, c +3:c*2 +3, :]   = img_crop_bb1
                # result[r+3:r*2 +3, c*2 +6:c*3 +6, :] = img_crop2
                # result[r+3:r*2 +3, c*3 +9:, :]       = img_crop_bb2
                # result[r*2 +6:, :c, :]               = img_crop3
                # result[r*2 +6:, c+3:c*2 +3:, :]      = img_crop_bb3
                # result[r*2 +6:, c*2 +6:c*3 +6, :]    = img_crop4
                # result[r*2 +6:, c*3 + 9 :, :]        = img_crop_bb4
                # show_img(f"Risultato cropping {img_name}", result)
